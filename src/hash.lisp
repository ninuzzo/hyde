...
#|
Mime-type map.
This should be defined here because otherwise hash-to-list won't be
available at reading time. After all, it is a kinda constant hash, so it
...
(defconstant +mime-type-map+ #.(list-to-hash '(
  #|
  To sort this list in VIM use :'a,.sort i assuming you have marked the first
  line with ma and the current line is the last. You can also select the list
  typing V on the first line, then /^) , return, k and type :sort i

  Just insert here the full output from convert-mime.types.lisp
  That program converts Apache mime.types file in Lisp hash initialization
  syntax.
  |#
